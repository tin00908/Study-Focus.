            ...
            VStack(alignment: .leading, spacing: 22) {
                HStack(alignment: .top) {
                    ...
                    Button { showSettings = true } label: {
                        ZStack {
                            Circle().fill(store.accent.opacity(0.15)).frame(width: 44, height: 44)
                            Image(systemName: "gearshape.fill").foregroundStyle(store.accent)
                        }
                    }
                }
                ...
                VStack(spacing: 0) {
                    if store.todayEvents.isEmpty {
                        ContentUnavailableView("Nothing scheduled", systemImage: "calendar", description: Text("Add an event in Calendar."))
                            .padding(.vertical, 20)
                    }
                    ForEach(Array(store.todayEvents.enumerated()), id: \.element.id) { index, event in
                        HStack(spacing: 14) {
                            Text(event.date.formatted(date: .omitted, time: .shortened))
                                .font(.caption.monospacedDigit())
                                .foregroundStyle(.secondary)
                                .frame(width: 52, alignment: .leading)
                            RoundedRectangle(cornerRadius: 2).fill(event.tint).frame(width: 4, height: 35)
                        ...
                        .padding(.vertical, 11)
                        if index < store.todayEvents.count - 1 { Divider().padding(.leading, 66) }
                    }
                }
...
struct FocusHero: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 15) {
            HStack {
                Label("TODAY'S GOAL", systemImage: "target").font(.caption.weight(.bold))
                Spacer()
                Text("\(store.focusMinutes) / \(store.dailyFocusGoal) min").font(.caption.weight(.semibold))
            }
            ProgressView(value: Double(store.focusMinutes), total: Double(store.dailyFocusGoal))
                .tint(.white)
                .scaleEffect(y: 1.7)
            HStack(alignment: .bottom) {
                VStack(alignment: .leading) {
                    Text("Keep your momentum").font(.title3.bold())
                    Text("You're \(Int(store.focusGoalProgress * 100))% through your goal.")
                        .font(.caption)
                        .opacity(0.8)
                }
                Spacer()
        ...
        .padding(20)
        .background(LinearGradient(colors: [store.accent, store.accent.opacity(0.68)], startPoint: .topLeading, endPoint: .bottomTrailing), in: RoundedRectangle(cornerRadius: 24))
    }
}
    ...
    var body: some View {
        HStack {
            ...
            if let onAction {
                ...
                Text(action).font(.subheadline.weight(.medium)).foregroundStyle(.indigo)
            }
        }
    }
}
...
struct FocusTimerView: View {
    ...
    private let modes = ["Pomodoro", "Short break", "Long break", "Custom"]

    private var isStudyMode: Bool {
        selection != "Short break" && selection != "Long break"
    }

    private var duration: Int {
        switch selection {
        case "Pomodoro": 25 * 60
        case "Short break": 5 * 60
        case "Long break": 15 * 60
        default: customMinutes * 60
        }
    }
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 25) {
                Picker("Timer type", selection: $selection) {
                ...
                if selection == "Custom" {
                    Stepper("\(customMinutes) minutes", value: $customMinutes, in: 10...120, step: 5)
                ...
                Button(action: toggleTimer) {
                    Label(isRunning ? "Pause session" : "Start focusing", systemImage: isRunning ? "pause.fill" : "play.fill")
                        .fontWeight(.bold)
                        .frame(maxWidth: .infinity)
                        .padding()
                        .background(store.accent, in: Capsule())
                        .foregroundStyle(.white)
                }
                ...
                Toggle(isOn: $locked) {
                    Label {
                        VStack(alignment: .leading) {
                            Text("Lock interface")
                            Text("Shows a full-screen focus view while the timer runs.")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                    } icon: {
        ...
        .onChange(of: locked) { _, value in
            if !value { showFocusLock = false }
            if value && isRunning { showFocusLock = true }
        }
        .onReceive(ticker) { _ in
            ...
            if seconds > 1 {
                ...
                showCompletion = true
                if isStudyMode {
                    store.recordFocusSession(minutes: duration / 60, subject: subject)
                }
            }
        }
        .alert("Session complete!", isPresented: $showCompletion) {
            ...
            Text(isStudyMode ? "Nice work. Your \(subject.rawValue) focus time has been saved." : "Break complete.")
        }
        .fullScreenCover(isPresented: $showFocusLock) {
            FocusLockView(seconds: $seconds, isRunning: $isRunning) {
                showFocusLock = false
            }
    ...
    private var timerDial: some View {
        ZStack {
            ...
            Circle()
                .trim(from: 0, to: max(0.05, Double(seconds) / Double(duration)))
                .stroke(store.accent, style: StrokeStyle(lineWidth: 15, lineCap: .round))
                .rotationEffect(.degrees(-90))
            VStack(spacing: 6) {
                Text(timeString)
                    .font(.system(size: 52, weight: .bold, design: .rounded))
                    .monospacedDigit()
                Text(isRunning ? "Deep focus in progress" : "Ready when you are")
                    .foregroundStyle(.secondary)
            }
        }
    ...
    private func toggleTimer() {
        isRunning.toggle()
        if isRunning && locked { showFocusLock = true }
    }
...
struct FocusLockView: View {
    ...
    var body: some View {
        VStack(spacing: 28) {
            ...
            Text("Stay focused").font(.title.bold()).foregroundStyle(.white)
            ...
            Button(isRunning ? "Pause" : "Resume") { isRunning.toggle() }
                ...
                .tint(.white)
                .foregroundStyle(.indigo)
            Button("Unlock interface", action: unlock).foregroundStyle(.white.opacity(0.8))
            Spacer()
        }
...
struct AssignmentsView: View {
    ...
    private var results: [Assignment] {
        store.assignments
            .filter { assignment in
                let matchesSearch = search.isEmpty || assignment.title.localizedCaseInsensitiveContains(search)
                let matchesSubject = filter == nil || assignment.subject == filter
                return matchesSearch && matchesSubject
            }
            .sorted { first, second in
                sortHighFirst ? priorityValue(first.priority) > priorityValue(second.priority) : first.due < second.due
            }
    }
    ...
    var body: some View {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack {
                    ...
                    ForEach(Subject.allCases) { subject in
                        FilterChip(title: subject.rawValue, selected: filter == subject) { filter = subject }
                    }
                }
                .padding(.horizontal)
                .padding(.vertical, 8)
        ...
        .searchable(text: $search, prompt: "Search assignments")
        .toolbar {
            ToolbarItem(placement: .topBarLeading) {
                Menu {
                    Button("Priority") { sortHighFirst = true }
                    Button("Deadline") { sortHighFirst = false }
        ...
        .sheet(isPresented: $showAdd) { AddAssignmentView() }
    }
...
struct FilterChip: View {
    ...
    var body: some View {
        Button(title, action: action)
            ...
            .padding(.vertical, 8)
            .background(selected ? Color.indigo : Color.primary.opacity(0.08), in: Capsule())
...
struct TaskRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                Text(task.title).strikethrough(task.done).fontWeight(.semibold)
                HStack {
                    Label(task.subject.rawValue, systemImage: task.subject.icon)
                    Text("•")
            ...
            Text(task.priority.rawValue)
                ...
                .padding(.vertical, 5)
                .background(task.priority.color.opacity(0.13), in: Capsule())
        }
        .padding(14)
        .background(.background, in: RoundedRectangle(cornerRadius: 16))
    }
...
struct StatisticsView: View {
    ...
    @State private var showReadingUpdate = false
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                ...
                SectionLabel(title: "Goals", action: "Update reading") { showReadingUpdate = true }
        ...
        .sheet(isPresented: $showReadingUpdate) { ReadingUpdateView() }
    }
}

struct ReadingUpdateView: View {
    @Environment(\.dismiss) private var dismiss
    @EnvironmentObject private var store: StudyViewModel
    ...
    var body: some View {
        NavigationStack {
            Form {
                Stepper("Pages to add: \(pages)", value: $pages, in: 1...100)
                Text("Current total: \(store.readingPagesToday) pages")
                    .foregroundStyle(.secondary)
            }
